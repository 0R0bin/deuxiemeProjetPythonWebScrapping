...
#   ===============================
urlMultPage = 'index.html'
urlBase = 'http://books.toscrape.com/catalogue/'                                                               # URL à compléter avec celle du livre
urlCategoryPage = 'https://books.toscrape.com/catalogue/category/books/travel_2/' + urlMultPage        # URL de la catégorie que nous sommes entrain de traiter
...
if rCategoryPage.ok:   
    ...
    autrePage = soup.find('ul', {'class': 'pager'})
    if autrePage == None:
        print("Pas d'autres pages trouvées")
    else:
        autrePage = autrePage.find('a').get('href')
        urlMultPage = autrePage
        boolAutrePage = True
